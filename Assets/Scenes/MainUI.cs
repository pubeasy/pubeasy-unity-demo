...
public class MainUI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        PubeasyAds.Instance().SetCnServer(onlyCn);
    ...
    void LoadSplash()
    {
        ...
        PubeasySplash.Instance().LoadSplashAd(splashUnitId, extra);
        PubeasySplash.Instance().EntrySplashAdScenario(splashUnitId, "EntrySplashAdScenario");
    }
    ...
    private void OnApplicationPause(bool pause)
    {
        ...
        {
            splashUnitId = Configure.Instance().SplashUnitId;
            if (PubeasySplash.Instance().SplashAdReady(splashUnitId) && !splashShow)
            {
                splashShow = true;
                PubeasySplash.Instance().ShowSplashAd(splashUnitId, "SplashAdScenarioId");
            }
            else
            {
                LoadSplash();
    ...
    private void OnEnable()
    {
        ...
        PubeasySplash.Instance().OnSplashZoomOutStart += OnZoomOutStart;
        PubeasySplash.Instance().OnSplashZoomOutEnd += OnZoomOutEnd;
        PubeasySplash.Instance().OnSplashSkip += OnSkip;
        PubeasySplash.Instance().OnSplashAllLoaded += OnAllLoaded;


#if UNITY_ANDROID
        PubeasySplash.Instance().OnDownloadStart += OnDownloadStart;
        PubeasySplash.Instance().OnDownloadUpdate += OnDownloadUpdate;
        PubeasySplash.Instance().OnDownloadFinish += OnDownloadFinish;
        PubeasySplash.Instance().OnDownloadFailed += OnDownloadFailed;
        PubeasySplash.Instance().OnDownloadPause += OnDownloadPause;
        PubeasySplash.Instance().OnInstalled += OnInstallled;
#endif
    }
