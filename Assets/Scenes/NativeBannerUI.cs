...
public class NativeBannerUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (!editMode)
        {
            ...
            if (GUILayout.Button("加载"))
            {
                ...
                PubeasyNativeBanner.Instance().LoadNativeBannerAd(adUnitId, sceneId,extra);
                ...
                PubeasyNativeBanner.Instance().SetCustomAdInfo(adUnitId, customAdInfo);
            }
            GUILayout.Space(20);
            if (GUILayout.Button("isReady"))
            {
                bool isReady = PubeasyNativeBanner.Instance().NativeBannerAdReady(adUnitId);
            ...
            if (GUILayout.Button("展示"))
            {
                ...
                infoStr = "";
                PubeasyNativeBanner.Instance().ShowNativeBannerAd(adUnitId, sceneId);
            }
            GUILayout.Space(20);
            ...
            {
                infoStr = "已隐藏";
                PubeasyNativeBanner.Instance().HideNativeBanner(adUnitId);
            }

            if (GUILayout.Button("显示"))
            {
                infoStr = "取消隐藏";
                PubeasyNativeBanner.Instance().DisplayNativeBanner(adUnitId);
            }

            if (GUILayout.Button("销毁"))
            {
                infoStr = "已销毁";
                PubeasyNativeBanner.Instance().DestroyNativeBanner(adUnitId);
            }
            ...
            {
                PubeasyNativeBanner.Instance().EntryNativeBannerAdScenario(adUnitId, sceneId);
            }
            GUILayout.Space(20);
    ...
    {
        //常用
        PubeasyNativeBanner.Instance().OnNativeBannerLoaded += OnlLoaded;
        PubeasyNativeBanner.Instance().OnNativeBannerLoadFailed += OnLoadFailed;
        PubeasyNativeBanner.Instance().OnNativeBannerImpression += OnImpression;
        PubeasyNativeBanner.Instance().OnNativeBannerShowFailed += OnShowFailed;
        PubeasyNativeBanner.Instance().OnNativeBannerClicked += OnClicked;
        PubeasyNativeBanner.Instance().OnNativeBannerOneLayerLoadFailed += OnOneLayerLoadFailed;
        //
        PubeasyNativeBanner.Instance().OnNativeBannerStartLoad += OnStartLoad;
        PubeasyNativeBanner.Instance().OnNativeBannerBiddingStart += OnBiddingStart;
        PubeasyNativeBanner.Instance().OnNativeBannerBiddingEnd += OnBiddingEnd;
        PubeasyNativeBanner.Instance().OnNativeBannerOneLayerStartLoad += OnOneLayerStartLoad;
        PubeasyNativeBanner.Instance().OnNativeBannerOneLayerLoaded += OnOneLayerLoaded;
        PubeasyNativeBanner.Instance().OnNativeBannerAllLoaded += OnAllLoaded;


#if UNITY_ANDROID

        PubeasyNativeBanner.Instance().OnDownloadStart += OnDownloadStart;
        PubeasyNativeBanner.Instance().OnDownloadUpdate += OnDownloadUpdate;
        PubeasyNativeBanner.Instance().OnDownloadFinish += OnDownloadFinish;
        PubeasyNativeBanner.Instance().OnDownloadFailed += OnDownloadFailed;
        PubeasyNativeBanner.Instance().OnDownloadPause += OnDownloadPause;
        PubeasyNativeBanner.Instance().OnInstalled += OnInstallled;
#endif
    }
