...
public class PrivacyUI : MonoBehaviour
{
    ...
    void checkInfo()
    {
        lgpdState = PubeasyAds.Instance().GetLGPDConsent();
        ...
        gdprState =  PubeasyAds.Instance().GetGDPRDataCollection();
        ...
        ccpaState = PubeasyAds.Instance().GetCCPADoNotSell();
        ...
        coppaState = PubeasyAds.Instance().GetCOPPAIsAgeRestrictedUser();
        ...
        openPersonalized = PubeasyAds.Instance().IsOpenPersonalizedAd();
    ...
    private void OnGUI()
    {
        ...
        GUILayout.Space(20);
        GUILayout.Label("PluginVersion :"+ PubeasyAds.PluginVersion);
        GUILayout.Label("SDKVersion :" + PubeasyAds.Instance().Version());
        GUILayout.Label("是否在欧洲 :" + PubeasyAds.Instance().IsEUTraffic());
        GUILayout.Label("是否在加州 :" + PubeasyAds.Instance().IsCalifornia());
        ...
        if(gdprState != 0)
        {
            ...
            {
                //设置 GDPR
                PubeasyAds.Instance().SetGDPRDataCollection(true);
                checkInfo();
            }
        }
        else
        {
            ...
            {
                //设置 GDPR
                PubeasyAds.Instance().SetGDPRDataCollection(false);
                checkInfo();
            }
        }
        ...
        if (lgpdState != 0)
        {
            if (GUILayout.Button("设置为 允许上报"))
            {
                PubeasyAds.Instance().SetLGPDConsent(true);
                checkInfo();
            }
        }
        else
        {
            if (GUILayout.Button("设置为 不允许上报"))
            {
                PubeasyAds.Instance().SetLGPDConsent(false);
                checkInfo();
            }
        }
        GUILayout.EndHorizontal();
        ...
        if (ccpaState != 0)
        {
            ...
            {
                //设置 CCPA
                PubeasyAds.Instance().SetCCPADoNotSell(true);
                checkInfo();
            }
        }
        else
        {
            ...
            {
                //设置 CCPA
                PubeasyAds.Instance().SetCCPADoNotSell(false);
                checkInfo();
            }
        }
        GUILayout.EndHorizontal();
        ...
        if (coppaState != 0)
        {
            ...
            {
                //设置 COPPA
                PubeasyAds.Instance().SetCOPPAIsAgeRestrictedUser(true);
                checkInfo();
            }
        }
        else
        {
            ...
            {
                //设置 COPPA
                PubeasyAds.Instance().SetCOPPAIsAgeRestrictedUser(false);
                checkInfo();
            }
        }
        GUILayout.EndHorizontal();
        ...
        if (openPersonalized == false)
        {
            ...
            {
                //设置 国内个性化推荐
                PubeasyAds.Instance().SetOpenPersonalizedAd(true);
                checkInfo();
            }
        }
        else
        {
            ...
            {
                //设置 国内个性化推荐
                PubeasyAds.Instance().SetOpenPersonalizedAd(false);
                checkInfo();
            }
        }
        GUILayout.EndHorizontal();
        ...
        {
            //调用 GDRP授权页面
            PubeasyAds.Instance().ShowGDPRDialog();
        }
    ...
    {
        PubeasyAds.Instance().OnDialogClosed += OnDialogClosed;
