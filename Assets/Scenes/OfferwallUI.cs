...
public class OfferwallUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("加载"))
        {
            ...
            PubeasyOfferwall.Instance().LoadOfferwallAd(adUnitId, extra);
            ...
            PubeasyOfferwall.Instance().SetCustomAdInfo(adUnitId, customAdInfo);
        }
        GUILayout.Space(20);
        if (GUILayout.Button("isReady"))
        {
            bool isReady = PubeasyOfferwall.Instance().OfferwallAdReady(adUnitId);
        ...
        if (GUILayout.Button("展示"))
        {
            ...
            PubeasyOfferwall.Instance().ShowOfferwallAd(adUnitId, sceneId);
        }
        GUILayout.Space(20);
        ...
        {
            PubeasyOfferwall.Instance().GetCurrencyBalance(adUnitId);
        }

        if (GUILayout.Button("设置用户名"))
        {
            PubeasyOfferwall.Instance().SetUserId(adUnitId, "test_offerwall_userid");
        }
        ...
        {
            PubeasyOfferwall.Instance().AwardBalance(adUnitId,20);
        }

        if (GUILayout.Button("扣除积分 10"))
        {
            PubeasyOfferwall.Instance().SpendBalance(adUnitId, 10);
        }
        GUILayout.EndHorizontal();
        ...
        {
            PubeasyOfferwall.Instance().EntryOfferwallAdScenario(adUnitId, sceneId);
        }
        GUILayout.Space(20);
    ...
    {
        //常用
        PubeasyOfferwall.Instance().OnOfferwallLoaded += OnlLoaded;
        PubeasyOfferwall.Instance().OnOfferwallLoadFailed += OnLoadFailed;
        PubeasyOfferwall.Instance().OnOfferwallImpression += OnImpression;
        PubeasyOfferwall.Instance().OnOfferwallShowFailed += OnShowFailed;
        PubeasyOfferwall.Instance().OnOfferwallClicked += OnClicked;
        PubeasyOfferwall.Instance().OnOfferwallClosed += OnClosed;
        PubeasyOfferwall.Instance().OnOfferwallOneLayerLoadFailed += OnOneLayerLoadFailed;
        //
        PubeasyOfferwall.Instance().OnOfferwallStartLoad += OnStartLoad;
        PubeasyOfferwall.Instance().OnOfferwallOneLayerStartLoad += OnOneLayerStartLoad;
        PubeasyOfferwall.Instance().OnOfferwallOneLayerLoaded += OnOneLayerLoaded;
        PubeasyOfferwall.Instance().OnOfferwallAllLoaded += OnAllLoaded;

        PubeasyOfferwall.Instance().OnOfferwallSetUserIdFinish += OnSetUserIdFinish;
        PubeasyOfferwall.Instance().OnCurrencyBalanceSuccess += OnCurrencyBalanceSuccess;
        PubeasyOfferwall.Instance().OnCurrencyBalanceFailed += OnCurrencyBalanceFailed;
        PubeasyOfferwall.Instance().OnSpendCurrencySuccess += OnSpendCurrencySuccess;
        PubeasyOfferwall.Instance().OnSpendCurrencyFailed += OnSpendCurrencyFailed;
        PubeasyOfferwall.Instance().OnAwardCurrencySuccess += OnAwardCurrencySuccess;
        PubeasyOfferwall.Instance().OnAwardCurrencyFailed += OnAwardCurrencyFailed;
