...
namespace PubeasySDK.Android
{
    ...
    public class PubeasyInterActiveAndroid
    {
        ...
        private static PubeasyInterActiveAndroid _instance;

        public static PubeasyInterActiveAndroid Instance()
        {
            if (_instance == null)
            {
                _instance = new PubeasyInterActiveAndroid();
            }
            return _instance;
        }
        ...
        private class AdLoadListenerAdapter : AndroidJavaProxy
        {
            ...
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveLoaded(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }

            void onAdClicked(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveClicked(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }

            void onAdImpression(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveImpression(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }// 展示 1300

            void onAdLoadFailed(string unitId, string error)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveLoadFailed(unitId, (Dictionary<string, object>)Json.Deserialize(error));
            }

            void onAdClosed(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveClosed(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            } // 关闭 1400
            ...
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveAllLoaded(unitId, b);

            }

            void oneLayerLoadFailed(string unitId, string tperror, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveOneLayerLoadFailed(unitId, (Dictionary<string, object>)Json.Deserialize(tperror), (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void oneLayerLoaded(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveOneLayerLoaded(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void onAdStartLoad(string unitId)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveStartLoad(unitId, null);

            }

            void oneLayerLoadStart(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveOneLayerStartLoad(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void onBiddingStart(string unitId, string tpAdInfo)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveBiddingStart(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void onBiddingEnd(string unitId, string tpAdInfo, string tpAdError)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveBiddingEnd(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo), (Dictionary<string, object>)Json.Deserialize(tpAdError));

            }

            void onAdIsLoading(string unitId)
            {
                PubeasyInterActiveAndroid.Instance().OnInterActiveIsLoading(unitId);

            }

        }

        public PubeasyInterActiveAndroid()
        {
        }
