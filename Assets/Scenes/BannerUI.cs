...
public class BannerUI : MonoBehaviour
{
    ...
    PubeasyBase.AdContentMode contentMode = PubeasyBase.AdContentMode.Top;
    string xStr = "0";
    string yStr = "0";
    string widthStr = "320";
    string heightStr = "50";
    ...
    private void OnGUI()
    {
        ...
        if (!editMode)
        {
            ...
            if (GUILayout.Button("加载"))
            {
                ...
                PubeasyBanner.Instance().LoadBannerAd(adUnitId, sceneId, extra);
                ...
                PubeasyBanner.Instance().SetCustomAdInfo(adUnitId, customAdInfo);
            }
            GUILayout.Space(20);
            if (GUILayout.Button("isReady"))
            {
                bool isReady = PubeasyBanner.Instance().BannerAdReady(adUnitId);
            ...
            if (GUILayout.Button("展示"))
            {
                ...
                infoStr = "";
                PubeasyBanner.Instance().ShowBannerAd(adUnitId, sceneId);
            }
            GUILayout.Space(20);
            ...
            {
                infoStr = "已隐藏";
                PubeasyBanner.Instance().HideBanner(adUnitId);
            }

            if (GUILayout.Button("显示"))
            {
                infoStr = "取消隐藏";
                PubeasyBanner.Instance().DisplayBanner(adUnitId);
            }

            if (GUILayout.Button("销毁"))
            {
                infoStr = "已销毁";
                PubeasyBanner.Instance().DestroyBanner(adUnitId);
            }
            ...
            {
                PubeasyBanner.Instance().EntryBannerAdScenario(adUnitId, sceneId);
            }
            GUILayout.Space(20);
        ...
        else
        {
            ...
            GUILayout.Space(20);
            string contentStr = "";
            if (contentMode == PubeasyBase.AdContentMode.Top)
            {
                contentStr = "banner内居中模式：Top";
            }
            else if (contentMode == PubeasyBase.AdContentMode.Center)
            {
                contentStr = "banner内居中模式：Center";
            }
            else if (contentMode == PubeasyBase.AdContentMode.Bottom)
            {
            ...
            {
                contentMode = PubeasyBase.AdContentMode.Top;
            }
            if (GUILayout.Button("Center"))
            {
                contentMode = PubeasyBase.AdContentMode.Center;
            }
            if (GUILayout.Button("Bottom"))
            {
                contentMode = PubeasyBase.AdContentMode.Bottom;
            }
            GUILayout.EndHorizontal();
#endif
    ...
    private void OnEnable()
    {
        ...
        //
        PubeasyBanner.Instance().OnBannerStartLoad += OnStartLoad;
        PubeasyBanner.Instance().OnBannerBiddingStart += OnBiddingStart;
        PubeasyBanner.Instance().OnBannerBiddingEnd += OnBiddingEnd;
        PubeasyBanner.Instance().OnBannerOneLayerStartLoad += OnOneLayerStartLoad;
        PubeasyBanner.Instance().OnBannerOneLayerLoaded += OnOneLayerLoaded;
        PubeasyBanner.Instance().OnBannerAllLoaded += OnAllLoaded;

#if UNITY_ANDROID
        PubeasyBanner.Instance().OnDownloadStart += OnDownloadStart;
        PubeasyBanner.Instance().OnDownloadUpdate += OnDownloadUpdate;
        PubeasyBanner.Instance().OnDownloadFinish += OnDownloadFinish;
        PubeasyBanner.Instance().OnDownloadFailed += OnDownloadFailed;
        PubeasyBanner.Instance().OnDownloadPause += OnDownloadPause;
        PubeasyBanner.Instance().OnInstalled += OnInstallled;
#endif
    }
