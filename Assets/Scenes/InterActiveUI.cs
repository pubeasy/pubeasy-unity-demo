...
public class InterActiveUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (!editMode)
        {
            ...
            if (GUILayout.Button("加载"))
            {
                ...
                PubeasyInterActive.Instance().LoadInterActiveAd(adUnitId, extra);
                ...
                PubeasyInterActive.Instance().SetCustomAdInfo(adUnitId, customAdInfo);
            }
            GUILayout.Space(10);
            if (GUILayout.Button("isReady"))
            {
                bool isReady = PubeasyInterActive.Instance().InterActiveAdReady(adUnitId);
            ...
            if (GUILayout.Button("展示"))
            {
                ...
                infoStr = "";
                PubeasyInterActive.Instance().ShowInterActiveAd(adUnitId, sceneId);
            }
            GUILayout.Space(10);
            ...
            {
                infoStr = "已隐藏";
                PubeasyInterActive.Instance().HideInterActive(adUnitId);
            }

            if (GUILayout.Button("显示"))
            {
                infoStr = "取消隐藏";
                PubeasyInterActive.Instance().DisplayInterActive(adUnitId);
            }

            if (GUILayout.Button("销毁"))
            {
                infoStr = "已销毁";
                PubeasyInterActive.Instance().DestroyInterActive(adUnitId);
            }
    ...
    {
        //常用
        PubeasyInterActive.Instance().OnInterActiveLoaded += OnlLoaded;
        PubeasyInterActive.Instance().OnInterActiveLoadFailed += OnLoadFailed;
        PubeasyInterActive.Instance().OnInterActiveImpression += OnImpression;
        PubeasyInterActive.Instance().OnInterActiveShowFailed += OnShowFailed;
        PubeasyInterActive.Instance().OnInterActiveClicked += OnClicked;
        PubeasyInterActive.Instance().OnInterActiveClosed += OnClosed;
        PubeasyInterActive.Instance().OnInterActiveOneLayerLoadFailed += OnOneLayerLoadFailed;
        //
        PubeasyInterActive.Instance().OnInterActiveStartLoad += OnStartLoad;
        PubeasyInterActive.Instance().OnInterActiveBiddingStart += OnBiddingStart;
        PubeasyInterActive.Instance().OnInterActiveBiddingEnd += OnBiddingEnd;
        PubeasyInterActive.Instance().OnInterActiveIsLoading += OnAdIsLoading;

        PubeasyInterActive.Instance().OnInterActiveOneLayerStartLoad += OnOneLayerStartLoad;
        PubeasyInterActive.Instance().OnInterActiveOneLayerLoaded += OnOneLayerLoaded;
        PubeasyInterActive.Instance().OnInterActiveVideoPlayStart += OnVideoPlayStart;
        PubeasyInterActive.Instance().OnInterActiveVideoPlayEnd += OnVideoPlayEnd;
        PubeasyInterActive.Instance().OnInterActiveAllLoaded += OnAllLoaded;
