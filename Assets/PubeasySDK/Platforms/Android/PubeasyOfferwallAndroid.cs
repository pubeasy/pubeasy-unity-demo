...
namespace PubeasySDK.Android
{
    ...
    public class PubeasyOfferwallAndroid
    {
        ...
        private static PubeasyOfferwallAndroid _instance;

        public static PubeasyOfferwallAndroid Instance()
        {
            if (_instance == null)
            {
                _instance = new PubeasyOfferwallAndroid();
            }
            return _instance;
        }
        ...
        private class AdLoadListenerAdapter : AndroidJavaProxy
        {
            ...
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallLoaded(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }

            void onAdClicked(string unitId, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallClicked(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }

            void onAdImpression(string unitId, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallImpression(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            }// 展示 1300

            void onAdFailed(string unitId, string error)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallLoadFailed(unitId, (Dictionary<string, object>)Json.Deserialize(error));
            }

            void onAdClosed(string unitId, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallClosed(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));
            } // 关闭 1400
            ...
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallAllLoaded(unitId, b);
            ...
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallIsLoading(unitId);

            }

            void oneLayerLoadFailed(string unitId, string tperror, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallOneLayerLoadFailed(unitId, (Dictionary<string, object>)Json.Deserialize(tperror), (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void oneLayerLoaded(string unitId, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallOneLayerLoaded(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void onAdStartLoad(string unitId)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallStartLoad(unitId, null);

            }

            void oneLayerLoadStart(string unitId, string tpAdInfo)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallOneLayerStartLoad(unitId, (Dictionary<string, object>)Json.Deserialize(tpAdInfo));

            }

            void currencyBalanceSuccess(String unitId, int amount, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnCurrencyBalanceSuccess(unitId,amount,msg);
            }
            void currencyBalanceFailed(String unitId, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnCurrencyBalanceFailed(unitId, msg);

            }
            void spendCurrencySuccess(String unitId, int amount, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnSpendCurrencySuccess(unitId, amount, msg);

            }
            void spendCurrencyFailed(String unitId, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnSpendCurrencyFailed(unitId, msg);

            }
            void awardCurrencySuccess(String unitId, int amount, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnAwardCurrencySuccess(unitId, amount, msg);

            }
            void awardCurrencyFailed(String unitId, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnAwardCurrencyFailed(unitId, msg);

            }
            void setUserIdSuccess(String unitId)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallSetUserIdFinish(unitId,true);

            }
            void setUserIdFailed(String unitId, String msg)
            {
                PubeasyOfferwallAndroid.Instance().OnOfferwallSetUserIdFinish(unitId, false);

            }
        }


        public PubeasyOfferwallAndroid()
        {
        }
