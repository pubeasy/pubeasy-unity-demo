...
public class RewardedUI : MonoBehaviour
{
    ...
    private void OnGUI()
    {
        ...
        if (GUILayout.Button("加载"))
        {
            ...
            PubeasyRewardVideo.Instance().LoadRewardVideoAd(adUnitId,extra);
            ...
            PubeasyRewardVideo.Instance().SetCustomAdInfo(adUnitId, customAdInfo);
        }
        GUILayout.Space(20);
        if (GUILayout.Button("isReady"))
        {
            bool isReady = PubeasyRewardVideo.Instance().RewardVideoAdReady(adUnitId);
        ...
        if (GUILayout.Button("展示"))
        {
            ...
            PubeasyRewardVideo.Instance().ShowRewardVideoAd(adUnitId, sceneId);
        }
        GUILayout.Space(20);
        ...
        {
            PubeasyRewardVideo.Instance().EntryRewardVideoAdScenario(adUnitId, sceneId);
        }
        GUILayout.Space(20);
    ...
    private void OnEnable()
    {
        ...
        //再看一个
        PubeasyRewardVideo.Instance().OnPlayAgainImpression += OnPlayAgainImpression;
        PubeasyRewardVideo.Instance().OnPlayAgainReward += OnPlayAgainReward;
        PubeasyRewardVideo.Instance().OnPlayAgainClicked += OnPlayAgainClicked;
        PubeasyRewardVideo.Instance().OnPlayAgainVideoPlayStart += OnPlayAgainVideoPlayStart;
        PubeasyRewardVideo.Instance().OnPlayAgainVideoPlayEnd += OnPlayAgainVideoPlayEnd;


#if UNITY_ANDROID

        PubeasyRewardVideo.Instance().OnDownloadStart += OnDownloadStart;
        PubeasyRewardVideo.Instance().OnDownloadUpdate += OnDownloadUpdate;
        PubeasyRewardVideo.Instance().OnDownloadFinish += OnDownloadFinish;
        PubeasyRewardVideo.Instance().OnDownloadFailed += OnDownloadFailed;
        PubeasyRewardVideo.Instance().OnDownloadPause += OnDownloadPause;
        PubeasyRewardVideo.Instance().OnInstalled += OnInstallled;
#endif
    }
